...
ggplot(Titanic, aes(x = Pclass)) + geom_bar(aes(fill = as.factor(Survived)), position = "dodge") + theme_bw() + xlab("Clase barco")
```

En términos del análisis descriptivo de la base de datos,el número de sobrevivientes de la clase 1 corresponden a 91 de sexo femenino y 45 del sexo masculino. De la clase 2, se pueden observar que hay 70 mujeres sobrevivientes y 17 para el caso del sexo masculino. En cuanto a la clase 3, hay 72 mujeres sobrevivientes a la tragedia y 47 hombres. Por otra parte, en el caso de los no sobrevivientes, la clase 1 presentan 3 mujeres y 77 hombres que no aguantaron las consecuencias del fatídico accidente. En el caso de la clase 2, se observa que hay 6 mujeres y 91 hombres que no lograron sobrevivir. Mientras que en la clase 3 se observa una mayor cantidad de decesos ya que hay 72 mujeres y alrrededor de 300 hombres que perdieron la vida.Lo decrito anteriormente, puede evidenciarse gráficamente en la Figura \@ref(fig:titanic2).
...
ggplot(Titanic, aes(x = Sex)) + geom_bar(aes(fill = as.factor(Survived)), position = "dodge") + theme_bw() + xlab("Género")
```
